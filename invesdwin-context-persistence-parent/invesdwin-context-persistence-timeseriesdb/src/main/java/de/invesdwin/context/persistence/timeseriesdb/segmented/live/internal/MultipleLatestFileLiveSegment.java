...
@NotThreadSafe
public class MultipleLatestFileLiveSegment<K, V> implements ILiveSegment<K, V> {

    private static final boolean LARGE_COMPRESSOR = false;
    private static final int LAST_VALUE_HISTORY = 2;
    ...
    private static AtomicInteger decompressions = new AtomicInteger();

    public MultipleLatestFileLiveSegment(final SegmentedKey<K> segmentedKey,
    ...
    private SerializingCollection<V> newSerializingCollection() {
        ...
        final TextDescription name = new TextDescription("%s[%s]: newSerializingCollection()",
                MultipleLatestFileLiveSegment.class.getSimpleName(), segmentedKey);
