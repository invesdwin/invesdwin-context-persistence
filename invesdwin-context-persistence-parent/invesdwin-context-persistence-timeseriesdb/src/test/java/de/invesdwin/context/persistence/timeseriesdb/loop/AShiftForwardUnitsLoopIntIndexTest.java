...
// CHECKSTYLE:OFF
@NotThreadSafe
public class AShiftForwardUnitsLoopIntIndexTest extends ATest {
    //CHECKSTYLE:ON
    ...
    public void testShiftForward() {
        for (int shiftForwardUnits = 0; shiftForwardUnits < DATES.size(); shiftForwardUnits++) {
            for (int i = 0; i < DATES.size(); i++) {
                ...
                final AShiftForwardUnitsLoopIntIndex<FDate> loop = newLoop(request, shiftForwardUnits);
                loop.loop();
                ...
                final AShiftForwardUnitsLoopIntIndex<FDate> loopMinus = newLoop(requestMinus, shiftForwardUnits);
                loopMinus.loop();
                ...
                final AShiftForwardUnitsLoopIntIndex<FDate> loopPlus = newLoop(requestPlus, shiftForwardUnits);
                loopPlus.loop();
    ...
    private AShiftForwardUnitsLoopIntIndex<FDate> newLoop(final FDate request, final int shiftForwardUnits) {
