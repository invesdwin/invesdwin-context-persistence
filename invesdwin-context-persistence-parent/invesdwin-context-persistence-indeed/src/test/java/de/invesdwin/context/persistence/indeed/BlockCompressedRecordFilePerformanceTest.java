...
@NotThreadSafe
//@Ignore("manual test")
public class BlockCompressedRecordFilePerformanceTest extends ADatabasePerformanceTest {
    ...
    public void testBlockCompressedRecordFilePerformance() throws IOException, InterruptedException {
        final File directory = new File(ContextProperties.getCacheDirectory(),
                BlockCompressedRecordFilePerformanceTest.class.getSimpleName());
        ...
        try (BlockCompressedRecordFile.Writer<TimedDecimal> writer = new BlockCompressedRecordFile.Writer.Builder<TimedDecimal>(
                file, IndeedSerializer.valueOf(TimedDecimalSerde.GET)).build()) {
            int i = 0;
    ...
    private void readIterator(final File file) throws IOException, InterruptedException {
        ...
        for (int reads = 1; reads <= READS; reads++) {
            ...
            int count = 0;
            try (BlockCompressedRecordFile<TimedDecimal> reader = new BlockCompressedRecordFile.Builder<TimedDecimal>(
                    file, IndeedSerializer.valueOf(TimedDecimalSerde.GET), new SnappyCodec()).build()) {
