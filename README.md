...
- **ISynchronousChannel**: since LevelDB by design supports only one process that accesses the database (even though it does multithreading properly) the developers say one should build their own server frontend around the database to access it from other processes. This makes also sense in order to have only one process that keeps the database updated with expiration checks a single instance of `ATimeSeriesUpdater`. Though this creates the problem of how to build a IPC (Inter-Process-Communication) mechanism that does not slow down the data access too much. As a solution, this module provides a unidirectional channel (you should use one channel for requests and a separate one for responses on a peer to peer basis) for blazing fast IPC on a single computer with the following tools:
	...
	- **Performance**: here are some performance measurements with against in-process queue implementations (in a non-blocking usage with serialization involved for comparison) using one channel for requests and another separate one for responses, thus each record (of 10,000,000) involving two messages (becoming 20,000,000):
```
ArrayDeque                Records:   127.26/ms    Messages:   254.52/ms  in  78579 ms    => ~55% slower than Named Pipes
Named Pipes on TMPFS      Records:   263.80/ms    Messages:   527.60/ms  in  37908 ms    => why ~7% slower on TMPFS?
Named Pipes 		  Records:   281.15/ms    Messages:   562.30/ms  in  35568 ms    => using this as baseline
LinkedBlockingQueue       Records:  1310.27/ms    Messages:  2620.54/ms  in   7632 ms    => ~4.6 times faster than Named Pipes
Mapped Memory             Records:  3214.40/ms    Messages:  6428.80/ms  in   3111 ms    => ~11.4 times faster than Named Pipes
Mapped Memory on TMPFS    Records:  3802.28/ms    Messages:  7604.56/ms  in   2630 ms    => ~13.5 times faster than Named Pipes
```
