...
- **ADelegateChronicleMap**: if you need random access to large elements, then LevelDB might not bring the best performance because it will reorganize itself very often which makes insertions very slow. If you need the ordered values from LevelDB, then you can use it as an index and store your large entries in a [ChronicleMap](https://github.com/OpenHFT/Chronicle-Map) with this. It is essentially a persistent map with good performance characteristics. Per default the entries are compressed with LZ4 fast compression to keep inserts fast while reducing the file size. If insert performance is not so important, then you could switch to LZ4 high compression as in SerializingCollection to optimize for read performance. Since ChronicleMap supports synchronization over multiple processes, you can even use it for IPC by opening the same file in each process. The processes will see each others changes. Though to get faster communication you should look into synchronous channels as described further below.
