...
ezdb-ConcurrentSkipListMap       Reads (GetLatest):     985.03/ms  => 5.9 times faster
           CQEngine (OnHeap)     Reads (GetLatest):   2,614.99/ms  => ~15.6 times faster (using NavigableIndex directly; query with "ORDER BY key DESC" results in 314.2/ms which is ~1.9 times faster)
  ...
  ezdb-LevelDB-Java              Reads (Iterator):    2,125.29/ms  => using this as baseline
           CQEngine (OnHeap)     Reads (Iterator):    5,732.63/ms  => ~2.7 times faster (using NavigableIndex directly; query "all" results in 3,345.38/ms which is ~1.6 times faster)
