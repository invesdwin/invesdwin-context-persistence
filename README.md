...
```
          ...
          ezdb-LevelDB-JNI (Disk)        Writes (PutBatch):       63.33/ms  => ~72% slower
                  CQEngine (Memory)      Writes (PutBatch):       65.97/ms  => ~71% slower (no persistence)
 ...
 Indeed-RecordLogDirectory (Disk)        Writes (Append):        199.19/ms  => ~13% slower
         ...
         ezdb-LevelDB-Java (Disk)        Writes (PutBatch):      228.07/ms  => using this as baseline
           ...
           Indeed-MphTable (Disk)        Writes (WriteAll):      491.50/ms  => ~2.15 times as fast (immutable after creation)
             ...
             ezdb-BTreeMap (Heap)        Writes (Put):         1,048.22/ms  => ~4.6 times as fast (no persistence)
                  CQEngine (Heap)        Writes (PutBatch):    1,083.07/ms  => ~4.7 times as fast (no persistence)
             ...
             tkrzw-TreeDBM (Disk)        Writes (Put):         1,270.97/ms  => ~5.6 times as fast
              ...
              ezdb-TreeMap (Heap)        Writes (Put):         1,902.77/ms  => ~8.3 times as fast (no persistence)
ezdb-ConcurrentSkipListMap (Heap)        Writes (Put):         2,358.21/ms  => ~10.3 times as fast (no persistence)
    Indeed-BasicRecordFile (Disk)        Writes (Append):      2,403.85/ms  => ~10.5 times as fast
           FastUtilHashMap (Heap)        Writes (put):         2,796.50/ms  => ~12.3 times as fast (no persistence)
             AgronaHashMap (Heap)        Writes (put):         3,088.23/ms  => ~13.5 times as fast (no persistence)
Indeed-BlockCompRecordFile (Disk)        Writes (Append):      4,273.32/ms  => ~18.7 times as fast
         ...
         ConcurrentHashMap (Heap)        Writes (put):        10,260.62/ms  => ~45 times as fast (no persistence)
                  Caffeine (Heap)        Writes (put):        10,378.83/ms  => ~45.5 times as fast (no persistence)
                   HashMap (Heap)        Writes (put):        14,695.08/ms  => ~64.4 times as fast (no persistence)
