...
- **ISynchronousChannel**: since LevelDB by design supports only one process that accesses the database (even though it does multithreading properly) the developers say one should build their own server frontend around the database to access it from other processes. This makes also sense in order to have only one process that keeps the database updated with expiration checks a single instance of `ATimeSeriesUpdater`. Though this creates the problem of how to build a IPC (Inter-Process-Communication) mechanism that does not slow down the data access too much. As a solution, this module provides a unidirectional channel (you should use one channel for requests and a separate one for responses on a peer to peer basis) for blazing fast IPC on a single computer with the following tools:
	...
	- **ILazySerdeValue**: there is a problem when reading `byte[]` entries from the database, deserializing them, filtering them by a timestamp according to a requested chunk, maybe transforming them, then serializing them for pushing them through the channel, just to deserialize them again on the other end. It is wasted CPU time to deserialize twice, so it would be handy to only optionally deserialize on the server (then filtering and transformation happens on the client side) or at least remembering the serialized `byte[]` entries to reuse them when sending them through the channel (when server side filtering on the deserialized values is still faster because less data will get transferred). To gain some flexibility here, you should wrap values with a `ILazySerdeValue` implementation of your choice and put that as the actual generic value into your database. Just wrap the `Serde` of the database with `LazySerdeValueSerde` and you will be able to remember the `byte[]` entries even though the database implementations provide no access to the bytes anymore. Alternatively you could just put `byte[]` into your database signature, but it is not nice to have to remember what Serde to use when client code needs the object, also it might be hard to track all the places where the value gets deserialized in a chain of methods, which might slow you down because of too many deserializations.
