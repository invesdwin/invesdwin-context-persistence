...
### invesdwin-context-persistence-mapdb
...
Indeed-RecordLogDirectory       Writes (Append):        199.19/ms  => ~13% slower
  ezdb-LevelDB-Java             Writes (PutBatch):      228.07/ms  => using this as baseline
    ...
    Indeed-MphTable             Writes (WriteAll):      491.50/ms  => ~2.15 times as fast (immutable after creation)
     Indeed-LsmTree             Writes (Put):           773.40/ms  => ~3.4 times as fast
      ezdb-BTreeMap             Writes (Put):         1,048.22/ms  => ~4.6 times as fast (no persistence)
       ...
       ChronicleMap (Disk)      Writes (Put):         2,275.73/ms  => ~10 times as fast (slower if entries are estimated wrong)
...
Indeed-BasicRecordFile          Writes (Append):      2,403.85/ms  => ~10.5 times as fast
       ChronicleMap (Memory)    Writes (Put):         2,940.31/ms  => ~12.9 times as fast (no persistence; slower if entries are estimated wrong)
Indeed-BlockCompRecordFile      Writes (Append):      4,273.32/ms  => ~18.7 times as fast
  ...
  ezdb-LevelDB-Java              Reads (Get):           244.29/ms  => using this as baseline
	      MapDB              Reads (Get):         1,142.33/ms  => ~4.7 times as fast
...
ezdb-ConcurrentSkipListMap       Reads (Get):         2,695.42/ms  => ~11 times as fast
    Indeed-MphTable              Reads (Get):         2,773.93/ms  => ~11.35 times as fast
      ezdb-BTreeMap              Reads (Get):         2,908.67/ms  => ~11.9 times as fast
       ...
       ChronicleMap (Disk)       Reads (Get):         5,759.47/ms  => ~23.6 times as fast (2-3 times faster for 10-100 times smaller databases)
           CQEngine (OffHeap)    Reads (Get):         5,813.95/ms  => ~23.8 times as fast
       ChronicleMap (Memory)     Reads (Get):         6,456.49/ms  => ~26.4 times as fast (2-3 times faster for 10-100 times smaller databases)
     Indeed-LsmTree              Reads (Get):         7,224.98/ms  => ~29.6 times as fast
   ...
   ezdb-LevelDB-JNI              Reads (GetLatest):      72.34/ms  => ~57% slower
     ...
     ezdb-TreeMapDB              Reads (GetLatest):     154.46/ms  => ~8% slower
  ezdb-LevelDB-Java              Reads (GetLatest):     167.48/ms  => using this as baseline
           CQEngine (OffHeap)    Reads (GetLatest):     300.70/ms  => ~80% faster (using NavigableIndex directly; query with "ORDER BY key DESC" results in 2,649.71/ms which is ~15.8 times faster)
           CQEngine (OnHeap)     Reads (GetLatest):     314.20/ms  => ~90% faster (using NavigableIndex directly; query with "ORDER BY key DESC" results in 2,614.99/ms which is ~15.6 times faster)
ezdb-ConcurrentSkipListMap       Reads (GetLatest):     985.03/ms  => ~5.9 times as fast
     ...
     Indeed-LsmTree              Reads (GetLatest):   2,937.29/ms  => ~17.5 times as fast
       ezdb-TreeMap              Reads (GetLatest):   3,235.20/ms  => ~19.3 times as fast
       
           CQEngine (Disk)       Reads (Iterator):      224.01/ms  => ~89.5% slower (using "Query All"; using NavigableIndex directly results in 7,939.03/ms which is ~3.7 times faster)
           CQEngine (OffHeap)    Reads (Iterator):      230.05/ms  => ~89.2% slower (using "Query All"; using NavigableIndex directly results in 7,669.30/ms which is ~3.6 times faster)
   ...
   ezdb-RocksDB-JNI              Reads (Iterator):      672.35/ms  => ~68.7% slower
     Indeed-LsmTree              Reads (Iterator):      879.74/ms  => ~58.6% slower (3 times as fast with 100 times smaller database)
  ezdb-LevelDB-Java              Reads (Iterator):    2,125.29/ms  => using this as baseline
     Indeed-LsmTree              Reads (Iterator):    2,581.38/ms  => ~21% faster (2 times as fast with 100 times smaller database)
...
Indeed-BlockCompRecordFile       Reads (Iterator):    9,285.14/ms  => ~4.37 times as fast
      ezdb-LMDB-JNR              Reads (Iterator):    9,330.80/ms  => ~4.4 times as fast
       ...
       ChronicleMap (Disk)       Reads (Iterator):   12,472.56/ms  => ~5.9 times as fast (unordered)
     ChronicleQueue              Reads (Iterator):   16,583.75/ms  => ~7.8 times as fast
       ChronicleMap (Memory)     Reads (Iterator):   18,460.40/ms  => ~8.7 times as fast (unordered)
