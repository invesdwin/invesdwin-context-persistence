...
```
  ...
  ezdb-LevelDB-Java             Writes (PutBatch):      228.07/ms  => using this as baseline
     ...
     ChronicleQueue             Writes (Append):        442.48/ms  => ~1.9 times as fast
...
ezdb-ConcurrentSkipListMap       Reads (Get):         2,695.42/ms  => ~11 times as fast
  ...
  ezdb-LevelDB-Java              Reads (GetLatest):     167.48/ms  => using this as baseline
           CQEngine (OffHeap)    Reads (GetLatest):     300.70/ms  => ~1.8 times as fast (using NavigableIndex directly; query with "ORDER BY key DESC" results in 2,649.71/ms which is ~15.8 times faster)
           CQEngine (OnHeap)     Reads (GetLatest):     314.20/ms  => ~1.9 times as fast (using NavigableIndex directly; query with "ORDER BY key DESC" results in 2,614.99/ms which is ~15.6 times faster)
ezdb-ConcurrentSkipListMap       Reads (GetLatest):     985.03/ms  => 5.9 times as fast
  ...
  ezdb-LevelDB-Java              Reads (Iterator):    2,125.29/ms  => using this as baseline
           CQEngine (OnHeap)     Reads (Iterator):    3,345.38/ms  => ~1.6 times as fast (using "Query All"; using NavigableIndex directly results in 5,732.63/ms which is ~2.7 times faster)
