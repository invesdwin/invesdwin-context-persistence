...
ezdb-ConcurrentSkipListMap      Writes (Put):         2,358.21/ms  => ~10.3 times faster (no persistence)
      ...
      ATimeSeriesDB (None)      Writes (Append):     34,069.23/ms  => ~149.4 times faster (Disabled Compression; 2x Size of ATimeSeriesDB with Compression)
...
ezdb-ConcurrentSkipListMap       Reads (Get):         2,695.42/ms  => ~11 times faster
    Indeed-MphTable              Reads (Get):         2,773.93/ms  => ~11.35 times faster
      ezdb-BTreeMap              Reads (Get):         2,908.67/ms  => ~11.9 times faster
       ezdb-TreeMap              Reads (Get):         4,875.67/ms  => ~20 times faster
           CQEngine (Disk)       Reads (Get):         5,656.11/ms  => ~23.15 times faster
           CQEngine (OnHeap)     Reads (Get):         5,732.63/ms  => ~23.5 times faster
       ChronicleMap (Disk)       Reads (Get):         5,759.47/ms  => ~23.6 times faster (2-3 times faster for 10-100 times smaller databases)
           CQEngine (OffHeap)    Reads (Get):         5,813.95/ms  => ~23.8 times faster
       ChronicleMap (Memory)     Reads (Get):         6,456.49/ms  => ~26.4 times faster (2-3 times faster for 10-100 times smaller databases)
  ...
  ezdb-LevelDB-Java              Reads (GetLatest):     167.48/ms  => using this as baseline
           CQEngine (OffHeap)    Reads (GetLatest):     300.70/ms  => ~1.8 times faster (using NavigableIndex directly; query with "ORDER BY key DESC" results in 2,649.71/ms which is ~15.8 times faster)
           CQEngine (OnHeap)     Reads (GetLatest):     314.20/ms  => ~1.9 times faster (using NavigableIndex directly; query with "ORDER BY key DESC" results in 2,614.99/ms which is ~15.6 times faster)
ezdb-ConcurrentSkipListMap       Reads (GetLatest):     985.03/ms  => 5.9 times faster
      ezdb-BTreeMap              Reads (GetLatest):   2,783.96/ms  => ~16.6 times faster
       ezdb-TreeMap              Reads (GetLatest):   3,235.20/ms  => ~19.3 times faster
  ...
  ezdb-LevelDB-Java              Reads (Iterator):    2,125.29/ms  => using this as baseline
     ...
     ChronicleQueue              Reads (Iterator):   16,583.75/ms  => ~7.8 times faster
       ChronicleMap (Memory)     Reads (Iterator):   18,460.40/ms  => ~8.7 times faster (unordered)
...
ezdb-ConcurrentSkipListMap       Reads (Iterator):   32,629.62/ms  => ~15.35 times faster
    Indeed-MphTable              Reads (Iterator):   34,411.56/ms  => ~16.2 times faster (unordered)
