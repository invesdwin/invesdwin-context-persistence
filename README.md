...
The `invesdwin-context-persistence-timeseries` module provides support for the popular [RocksDB](https://rocksdb.org/) which is based on the [LevelDB](https://github.com/google/leveldb) key-value storage with improved performance. The actual RocksDB/LevelDB API is very low level, so we use [EZDB](https://github.com/criccomini/ezdb) for a bit more comfort. Our module provides the following accessories:
