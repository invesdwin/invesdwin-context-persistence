  ...
  ezdb-LevelDB-Java             Writes (PutBatch):      228.07/ms  => using this as baseline
       InfluxDB-1.x             Writes (PutBatch):      252.08/ms  => ~10% faster
             Hsqldb (Disk)      Writes (PutBatch):      330.23/ms  => ~45% faster
                 H2 (Disk)      Writes (PutBatch):      338.59/ms  => ~48% faster
    ...
    Indeed-MphTable             Writes (WriteAll):      491.50/ms  => ~2.15 times as fast (immutable after creation)
      ...
      tkrzw-HashDBM             Writes (Put):           792.46/ms  => ~3.5 times as fast (speed degrades with larger tables)
      tkrzw-SkipDBM             Writes (Put):           872.30/ms  => ~3.8 times as fast (speed degrades with larger tables)
      ...
      tkrzw-TreeDBM             Writes (Put):         1,270.97/ms  => ~5.6 times as fast
...
Indeed-BasicRecordFile          Writes (Append):      2,403.85/ms  => ~10.5 times as fast
    FastUtilHashMap             Writes (put):         2,796.50/ms  => ~12.3 times as fast (no persistence)
      AgronaHashMap             Writes (put):         3,088.23/ms  => ~13.5 times as fast (no persistence)
Indeed-BlockCompRecordFile      Writes (Append):      4,273.32/ms  => ~18.7 times as fast
  ...
  ConcurrentHashMap             Writes (put):        10,260.62/ms  => ~45 times as fast (no persistence)
      ...
      ATimeSeriesDB (Fast)      Writes (Append):     30,629.75/ms  => ~134.3 times as fast (Fast Compression)
            QuestDB             Writes (Append):     36,191.23/ms  => ~158.7 times as fast (tested on Java 11; 4x Size of ATimeSeriesDB with Compression)
      ATimeSeriesDB (None)      Writes (Append):     43,744.53/ms  => ~191.8 times as fast (Disabled Compression; 2x Size of ATimeSeriesDB with Compression)
  ...
  ezdb-LevelDB-Java              Reads (Get):           244.29/ms  => using this as baseline
      ...
      tkrzw-HashDBM              Reads (Get):         2,528.45/ms  => ~10.3 times as fast
ezdb-ConcurrentSkipListMap       Reads (Get):         2,695.42/ms  => ~11 times as fast
    Indeed-MphTable              Reads (Get):         2,773.93/ms  => ~11.35 times as fast
      ...
      tkrzw-SkipDBM              Reads (Get):         6,035.00/ms  => ~24.7 times as fast
...
ezdb-ConcurrentSkipListMap       Reads (GetLatest):     985.03/ms  => ~5.9 times as fast
   ...
   ezdb-RocksDB-JNI              Reads (Iterator):      672.35/ms  => ~68.7% slower
      tkrzw-HashDBM              Reads (Iterator):      801.15/ms  => ~62.3% slower
       ezdb-LsmTree              Reads (Iterator):      879.74/ms  => ~58.6% slower
      tkrzw-TreeDBM              Reads (Iterator):    1,697.50/ms  => ~20.1% slower
  ezdb-LevelDB-Java              Reads (Iterator):    2,125.29/ms  => using this as baseline
                 H2 (Disk)       Reads (Iterator):    2,691.93/ms  => ~27% faster
...
ezdb-ConcurrentSkipListMap       Reads (Iterator):   32,629.62/ms  => ~15.35 times as fast
      ATimeSeriesDB (Fast)       Reads (Iterator):   34,292.38/ms  => ~16.1 times as fast (no caching)
    Indeed-MphTable              Reads (Iterator):   34,411.56/ms  => ~16.2 times as fast (unordered)
      ATimeSeriesDB (High)       Reads (Iterator):   34,494.65/ms  => ~16.2 times as fast (no caching)
      ATimeSeriesDB (None)       Reads (Iterator):   38,303.90/ms  => ~18 times as fast (no caching)
      AgronaHashMap              Reads (Iterator):   38,481.98/ms  => ~18.1 times as fast (unordered)
